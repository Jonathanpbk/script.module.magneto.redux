...
from magneto.trakt.api.base_cache import BASE_DELETE, connect_database
from magneto.trakt.api.lists_cache import lists_cache, lists_cache_object
...
from magneto.trakt.api.trakt_utils import clean_ids, sort_for_article, sort_list
from magneto.trakt.qr_dialog import QRProgressDialog
from magneto.trakt.qrcode_utils import make_qrcode
from magneto.trakt.compat import (
    ADDON_PATH,
    ADDON_VERSION,
    EMPTY_USER,
    cache_object,
...
class TraktBase:
    ...
    def ensure_token_valid(self):
        ...
        if expires and refresh_token:
            try:
                ...
                kodilog('Error checking token expiry: %s' % e)

    def no_client_key(self):
        notification('Trakt: Client key not set')
        return None

    def no_secret_key(self):
        notification('Trakt: Secret key not set')
        return None
    ...
    def _handle_unauthorized(self):
        ...
        notification('Trakt: Authorization expired', time=5000)
    ...
    def call_trakt(
        ...
        if response.status_code == 429:
            ...
            kodilog('Trakt Rate Limit Exceeded. Retrying in %s seconds.' % retry_after)
        ...
        try:
            ...
            error_message = error_messages.get(status_code, 'HTTP Error: %s' % status_code)
            kodilog('Trakt API error: %s' % response.text)
            raise ProviderException('Trakt API error: %s' % error_message) from error
        except requests.RequestException as error:
            raise ProviderException('Trakt API error: %s' % error) from error
    ...
    def get_trakt(self, params):
        try:
            ...
            kodilog('KeyError in get_trakt: %s' % e)
            raise
        except TypeError as e:
            kodilog('TypeError in get_trakt: %s' % e)
            raise
    ...
    def trakt_refresh_token(self):
        ...
        if response and isinstance(response, dict):
            ...
            set_property('trakt_expires', str(time.time() + 82800))
...
class TraktAuthentication(TraktBase):
    ...
    def trakt_get_device_token(self, device_codes):
        ...
        try:
            ...
            while not progressDialog.iscanceled and time_passed < expires_in:
                ...
                if status_code == 200:
                    result = response.json()
                    progressDialog.update_progress(100, 'Authentication successful!')
                    break
    ...
    def trakt_authenticate(self):
        ...
        if not token:
            ...
            notification('Trakt: Authentication failed', time=3000)
            return False
        ...
        try:
            ...
            if user and isinstance(user, dict):
                ...
                notification('Trakt: Authenticated as %s' % user['username'], time=3000)
                return True
        except Exception:
    ...
    def trakt_revoke_authentication(self):
        ...
        notification('Trakt: Logged out', time=3000)


class TraktMovies(TraktBase):
    def trakt_movies_trending(self, page_no):
        set_pluging_category('Trending Movies')
    ...
    def trakt_movies_trending_recent(self, page_no):
        set_pluging_category('Trending Recent Movies')
    ...
    def trakt_movies_top10_boxoffice(self):
        set_pluging_category('Box Office')
    ...
    def trakt_movies_most_watched(self, page_no):
        set_pluging_category('Most Watched Movies')
    ...
    def trakt_movies_most_favorited(self, page_no):
        set_pluging_category('Most Favorited Movies')
    ...
    def trakt_movies_progress(self):
        set_pluging_category('Movie Progress')
        try:
...
class TraktTV(TraktBase):
    def trakt_tv_trending(self, page_no):
        set_pluging_category('Trending Shows')
    ...
    def trakt_tv_trending_recent(self, page_no):
        set_pluging_category('Trending Recent Shows')
        current_year = get_datetime().year
        years = '%s-%s' % (current_year - 1, current_year)
    ...
    def trakt_tv_most_watched(self, page_no):
        set_pluging_category('Most Watched Shows')
    ...
    def trakt_tv_most_favorited(self, page_no):
        set_pluging_category('Most Favorited Shows')
...
class TraktLists(TraktBase):
    ...
    def _ids_match(candidate_ids, target_ids):
        for key in ('trakt', 'tmdb', 'tvdb', 'imdb'):
            ...
            target = target_ids.get(key) or target_ids.get('%s_id' % key)
    ...
    def trakt_comments(self, media_type, tmdb_id, sort_type='likes', page_no=1):
        ...
        params = {
            'path': '%s/%%s/comments/%%s' % media_type,
    ...
    def trakt_watched_history(self, media_type, page_no, sort_type='recent'):
        def _process(params, media_type):
            ...
            if sort_type == 'recent' or sort_type is None:
    ...
    def mark_as_watched(self, media_type, season, episode, ids):
        ...
        if (
            ...
            notification('Trakt: Already marked as watched', time=3000)
        else:
            notification('Trakt: Marked as watched', time=3000)
    ...
    def trakt_trending_popular_lists(self, list_type, page_no):
        set_pluging_category('Popular Lists')
...
class TraktScrobble(TraktBase):
    ...
    def _scrobble_payload(data):
        ...
        payload: Dict[str, Any] = {
            ...
            'app_date': _dt.now().strftime('%Y-%m-%d'),
        }
    ...
    def trakt_stop_scrobble(self, data):
        kodilog('Stopping Trakt scrobble')
        payload = self._scrobble_payload(data)
        if payload is None or payload.get('progress', 0) < 1.0:
            return
    ...
    def trakt_get_last_tracked_position(self, data):
        try:
            ...
            if cached_progress > 0:
                kodilog('Trakt: Found cached resume point: %s%%' % cached_progress)
                return cached_progress

            path = 'sync/playback/%s' % media_type
                    ...
                    try:
                        if item['type'] == 'movie':
                            ...
                            if trakt_tmdb == local_tmdb:
                                progress = item.get('progress', 0)
                                kodilog('Trakt: Found cloud resume point for movie: %s%%' % progress)
                                return progress
                            ...
                            if (
                                ...
                                progress = item.get('progress', 0)
                                kodilog('Trakt: Found cloud resume point for episode: %s%%' % progress)
                                return progress
                    except Exception:
                        continue

        except Exception as e:
            kodilog('Trakt: Error fetching last tracked position: %s' % e)
            return 0.0
    ...
    def trakt_get_playback_progress(self, media_type):
        try:
            ...
            kodilog('Error fetching playback progress: %s' % e)
            return []
