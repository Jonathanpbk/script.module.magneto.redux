...
# Namespace prefix for in-memory (window property) caching — must not clash with Magneto's own properties
media_prop = 'magneto.trakt.%s'
...
def check_databases_integrity():
    ...
    if database_errors:
        dialog_ok(
            ...
            line1='Corrupted databases were reset: %s' % ', '.join(database_errors),
        )
    else:
        notification('Trakt cache integrity OK', time=3000)
