...
DEBUGPREFIX = '[COLOR red][ MAGNETO %s ][/COLOR]'
...
def log(msg, caller=None, level=LOGINFO):
	...
	try:
		...
		if debug_location == '1':
			log_file = joinPath(LOGPATH, 'magneto.log')
...
def upload_LogFile():
	...
	try:
		...
		text = f.read()
		f.close()
		UserAgent = 'script.kodi.loguploader: 1.0' #'Magneto %s' % addonVersion()
		...
		if 'key' in response.json():
			...
			log('Magneto log file uploaded to: %s' % result)
			...
			select = selectDialog([i[0] for i in list], 'Magneto log file uploaded to:')
			...
			notification(message='Magneto Log upload failed: %s' % str(response.json()['message']))
			log('Magneto Log upload failed: %s' % str(response.json()['message']), level=LOGERROR)
		else:
			notification(message='Magneto Log upload failed')
			log('Magneto Log upload failed: %s' % response.text, level=LOGERROR)
	except:
		error('Magneto log upload failed')
